...
def get_industries(data):
    return sorted(data["industry"].unique())

def get_top_counties(data, selected_industry):
    # Filter dataset
    ...
    return [(row.county, row.predicted_score) for row in top5.itertuples()]
...
if __name__ == '__main__':
    ...
    top_counties = get_top_counties(data, selected_industry)

    # Output
    print("\nTop Counties for", selected_industry)

    for i, (county, score) in enumerate(top_counties, start=1):
        print(f"{i}. {county} (Score: {score:.3f})")
